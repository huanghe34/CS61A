...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        eval_within_env = (lambda env: lambda expr: scheme_eval(expr, env))(env)
        return scheme_apply(procedure, rest.map(eval_within_env), env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) < len(vals):
            raise SchemeError('too few operands in formals')
        elif len(formals) > len(vals):
            raise SchemeError('too many operands in formals')
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    next_val = scheme_eval(expressions.first, env)
    while scheme_truep(next_val) and expressions.second is not nil:
        expressions = expressions.second
        next_val = scheme_eval(expressions.first, env)
    return next_val
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    next_val = scheme_eval(expressions.first, env)
    while scheme_falsep(next_val) and expressions.second is not nil:
        expressions = expressions.second
        next_val = scheme_eval(expressions.first, env)
    return next_val
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    formal = lambda pair: pair.first
    val = lambda pair: scheme_eval(pair.second.first, env)
    check_binding = lambda pair: check_form(pair, 2, 2)
    bindings.map(check_binding)
    formals = bindings.map(formal)
    vals = bindings.map(val)
    check_formals(formals)
