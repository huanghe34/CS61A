...
def create_board(rows, columns):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [create_row(columns) for b in range(rows)]
...
def put_piece(board, max_rows, column, player):
    ...
    for i in reversed(range(max_rows)):
        if row == -1 and get_piece(board, i, column) == '-':
